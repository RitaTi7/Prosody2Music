"""
synth.py — Synth / Audio: renderizza Melody + Harmony in un file WAV.

Sintesi additiva semplice (parziali armoniche + inviluppo ADSR) in numpy
puro, così la pipeline produce audio ascoltabile senza dipendere da
soundfont/fluidsynth esterni (spesso non disponibili in ambienti sandbox).
"""

import numpy as np
from scipy.io import wavfile
from instruments import INSTRUMENT_PRESETS, DEFAULT_MELODY_INSTRUMENT, DEFAULT_HARMONY_INSTRUMENT

SAMPLE_RATE = 44100

# Profilo di sintesi per famiglia di strumenti: contenuto armonico
# (coppie [armonica, ampiezza_relativa]) + parametri ADSR caratteristici.
# Non è un vero campionamento, ma dà a ciascuna famiglia un carattere
# timbrico riconoscibile con la sola sintesi additiva.
TIMBRE_PROFILES = {
    "piano": {
        "harmonics": [(1, 1.0), (2, 0.5), (3, 0.25), (4, 0.12), (5, 0.06)],
        "adsr": dict(a=0.005, d=0.25, s=0.25, r=0.25),
    },
    "strings": {
        "harmonics": [(1, 1.0), (2, 0.3), (3, 0.1)],
        "adsr": dict(a=0.08, d=0.2, s=0.8, r=0.3),
    },
    "pluck": {
        # chitarra/marimba: attacco quasi istantaneo, decadimento rapido, poco sustain
        "harmonics": [(1, 1.0), (2, 0.6), (3, 0.35), (4, 0.2), (5, 0.1), (6, 0.05)],
        "adsr": dict(a=0.002, d=0.35, s=0.05, r=0.15),
    },
    "flute": {
        # spettro povero di armoniche (quasi sinusoidale), attacco morbido, poco vibrato
        "harmonics": [(1, 1.0), (2, 0.08), (3, 0.03)],
        "adsr": dict(a=0.06, d=0.1, s=0.85, r=0.12),
    },
    "organ": {
        # ricco di armoniche pari e dispari, sustain pieno senza decadimento (mantice)
        "harmonics": [(1, 1.0), (2, 0.6), (3, 0.5), (4, 0.35), (5, 0.25), (6, 0.15), (8, 0.1)],
        "adsr": dict(a=0.01, d=0.02, s=1.0, r=0.05),
    },
    "brass": {
        # spettro brillante, attacco marcato ma non percussivo
        "harmonics": [(1, 1.0), (2, 0.7), (3, 0.55), (4, 0.4), (5, 0.3), (6, 0.2)],
        "adsr": dict(a=0.03, d=0.1, s=0.75, r=0.15),
    },
    "bass": {
        # fondamentale dominante, poche armoniche alte, sustain lungo
        "harmonics": [(1, 1.0), (2, 0.35), (3, 0.1)],
        "adsr": dict(a=0.01, d=0.15, s=0.7, r=0.2),
    },
}


def midi_to_freq(pitch: int) -> float:
    return 440.0 * (2.0 ** ((pitch - 69) / 12.0))


def adsr_envelope(n_samples, sr=SAMPLE_RATE, a=0.01, d=0.08, s=0.7, r=0.15):
    a_n = int(a * sr)
    d_n = int(d * sr)
    r_n = int(r * sr)
    s_n = max(0, n_samples - a_n - d_n - r_n)
    env = np.concatenate([
        np.linspace(0, 1, max(a_n, 1)),
        np.linspace(1, s, max(d_n, 1)),
        np.full(max(s_n, 0), s),
        np.linspace(s, 0, max(r_n, 1)),
    ])
    if len(env) < n_samples:
        env = np.pad(env, (0, n_samples - len(env)))
    return env[:n_samples]


def render_tone(freq, duration_sec, velocity=80, sr=SAMPLE_RATE, timbre="piano"):
    n = max(1, int(duration_sec * sr))
    t = np.arange(n) / sr
    amp = (velocity / 127.0)

    profile = TIMBRE_PROFILES.get(timbre, TIMBRE_PROFILES["piano"])
    wave = np.zeros(n)
    for h, hamp in profile["harmonics"]:
        wave += hamp * np.sin(2 * np.pi * freq * h * t)
    env = adsr_envelope(n, sr, **profile["adsr"])

    wave = wave / max(np.max(np.abs(wave)), 1e-9)
    return wave * env * amp


def render_track(events, sr=SAMPLE_RATE, timbre="piano"):
    """
    events: lista di (pitch_or_list, start_sec, duration_sec, velocity)
    pitch_or_list può essere un intero (nota singola) o lista (accordo)
    """
    total_len = int(sr * (max((s + d for _, s, d, _ in events), default=1.0) + 0.5))
    track = np.zeros(total_len)
    for pitches, start, dur, vel in events:
        if isinstance(pitches, int):
            pitches = [pitches]
        for p in pitches:
            freq = midi_to_freq(p)
            tone = render_tone(freq, dur, velocity=vel, sr=sr, timbre=timbre)
            start_i = int(start * sr)
            end_i = start_i + len(tone)
            if end_i > len(track):
                track = np.pad(track, (0, end_i - len(track)))
            track[start_i:end_i] += tone
    return track


def mix_and_export(melody, harmony, tempo,
                    melody_instrument=DEFAULT_MELODY_INSTRUMENT,
                    harmony_instrument=DEFAULT_HARMONY_INSTRUMENT,
                    out_path="output.wav"):
    from instruments import validate_instrument
    melody_instrument = validate_instrument(melody_instrument)
    harmony_instrument = validate_instrument(harmony_instrument)
    melody_timbre = INSTRUMENT_PRESETS[melody_instrument]["timbre"]
    harmony_timbre = INSTRUMENT_PRESETS[harmony_instrument]["timbre"]

    beat_sec = 60.0 / tempo

    mel_events = []
    t = 0.0
    for note in melody.notes:
        dur_sec = note.duration * beat_sec
        mel_events.append((note.pitch, t, dur_sec * 0.92, note.velocity))
        t += dur_sec

    harm_events = []
    t = 0.0
    for chord in harmony.chords:
        dur_sec = chord.duration * beat_sec
        harm_events.append((chord.pitches, t, dur_sec * 0.98, 45))
        t += dur_sec

    mel_track = render_track(mel_events, timbre=melody_timbre)
    harm_track = render_track(harm_events, timbre=harmony_timbre)

    n = max(len(mel_track), len(harm_track))
    mel_track = np.pad(mel_track, (0, n - len(mel_track)))
    harm_track = np.pad(harm_track, (0, n - len(harm_track)))

    mix = mel_track * 0.75 + harm_track * 0.5
    peak = np.max(np.abs(mix))
    if peak > 0:
        mix = mix / peak * 0.9  # normalizzazione, evita clipping

    audio_i16 = (mix * 32767).astype(np.int16)
    wavfile.write(out_path, SAMPLE_RATE, audio_i16)
    return out_path


if __name__ == "__main__":
    from prosody import analyze_poem
    from emotion import analyze_emotion
    from music_transformer import MusicTransformer

    demo = "Nel mezzo del cammin di nostra vita\nmi ritrovai per una selva oscura"
    pa = analyze_poem(demo)
    em = analyze_emotion(demo)
    mt = MusicTransformer(seed=42)
    melody, harmony, meta = mt.generate(pa, em, text_seed=demo)
    path = mix_and_export(melody, harmony, meta["tempo"])
    print("scritto:", path)
