...
3) read the function prepbeg_nOfLet.R; To do that, write 	
	source("prepbeg_nOfLet.R")
	and press Enter
Now the function is loaded and you can run it. 	
	
	
4) Run the function. The function prepbeg_nOfLet.R has only one argument, i.e. a number from 1 to 4; the number specifies the length of word beginnings you are interested in. Therefore, if you are interested in looking at the word beginnings of 2 letters, you run the function with 2 as argument, i.e., prepbeg_nOfLet(2). The function creates and saves an object called db2.RData (where the number 2 indicates the argument you have specified in the function; this means that if you run, e.g.,  prepbeg_nOfLet(1), you will create an object called db1.RData). The object you create will be used later on. How to run the prepbeg_nOfLet function? Just write	
	prepbeg_nOfLet(n)
	and press Enter [NOTE that instead of n you have to write a number from 1 to 4; e.g., prepbeg_nOfLet(3)]
...
How to run the functions? Below some examples; Write	
	...
	beg("ba",1, syll=3, gCat="S")
	and press Enter
	
	beg("ba",1, gCat="S")
	and press Enter
	
	beg("ba",1, syll=3, gCat="S", cvStrFin="VCV")
	and press Enter
	
	beg("ba",1, syll=3, cvStrFin="VCV")
	and press Enter
...
3) read the script prepFinSeq.R; To do that, write 	
	source("prepFinSeq.R")
	and press Enter
The script creates an object (dbfSeq.RData) that is then used by the second function. 	
	
The function takes a while (you can see on your screen some numbers indicating the processing). Before doing step 4, wait until the function ends. 	
	
Note that you may read and run this script only the first time; once you have created the dbfSeq.RData object you can directly run the function finSeq.R	
	
	
4) Run the function finSeq.R; this function has 2 mandatory arguments (a, b) and 2 optional arguments (syll, gCat). If an argument is mandatory means that you must specify it in the function; differently, if an argument is optional you can decide whether you want or do not want to specify it. Therefore, when you run the function, you must specify at least 2 arguments (a,b) and, if you want, you may refine your research by specifying also some (or all) of the optional arguments.	
The arguments are:  	
...
5) If you want to do multiple searches, repeat step 6) as many times as you want.	
