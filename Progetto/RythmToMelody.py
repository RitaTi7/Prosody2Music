...
def main():
    global START_PITCH, SCALE, ADD_ACCOMPANIMENT

    args = parse_args()

    # Interattivo: mostra opzioni correnti e chiedi se l'utente vuole personalizzare
    print("\nOpzioni correnti:")
    print(f" - Genere: {args.genre}")
    print(f" - Ritmo: {args.rhythm}")
    print(f" - Aggiungi accompagnamento: {not args.no_accompaniment}")
    print(f" - Seed: {args.seed}")
    print(f" - Start pitch: {START_PITCH}")
    print(f" - Scala: {'minor' if SCALE==[0,2,3,5,7,8,10] else 'major'}")
    ...
    if choice == "y":
        # Genere
        try:
            genre_in = input(f"Genere (opzioni: {', '.join(GENRE_PRESETS.keys())}) [vuoto = {args.genre}]: ").strip()
        except Exception:
            genre_in = ""
        if genre_in:
            if genre_in in GENRE_PRESETS:
                args.genre = genre_in
            else:
                print("[WARNING] Genere non riconosciuto, mantenuto il precedente.")

        # Ritmo
        try:
            rhythm_in = input(f"Ritmo (opzioni: {', '.join(RHYTHM_PRESETS.keys())}) [vuoto = {args.rhythm}]: ").strip()
        except Exception:
            rhythm_in = ""
        if rhythm_in:
            if rhythm_in in RHYTHM_PRESETS:
                args.rhythm = rhythm_in
            else:
                print("[WARNING] Livello ritmico non riconosciuto, mantenuto il precedente.")

        # Accompaniment
        try:
            acc_in = input(f"Aggiungere accompagnamento? [Y/n] (attuale = {not args.no_accompaniment}): ").strip().lower()
        except Exception:
            acc_in = ""
        if acc_in == "n":
        ...
        # Seed
        try:
            seed_in = input(f"Seed (intero) [vuoto = {args.seed}]: ").strip()
        except Exception:
            seed_in = ""
        if seed_in:
            try:
                ...
                print("[WARNING] Seed non valido, ignorato.")

        # Start pitch
        try:
            sp_in = input(f"Start pitch MIDI (intero) [vuoto = {START_PITCH}]: ").strip()
        except Exception:
            sp_in = ""
        if sp_in:
            try:
                START_PITCH = int(sp_in)
            except ValueError:
                print("[WARNING] Start pitch non valido, mantenuto il precedente.")

        # Scala: major/minor
        try:
            scale_in = input("Scala (major/minor) [vuoto = major]: ").strip().lower()
        except Exception:
            scale_in = ""
        if scale_in == "minor":
